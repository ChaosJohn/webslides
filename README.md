...
- 目标仓库：页面会自动解析——部署在默认 `<owner>.github.io/<repo>/` Pages 时从域名+路径推导；使用自定义域名时用 `&repo=owner/repo` 覆盖；主仓无需任何操作。
...
2. **上传配置**：用自己的 Token；仓库归属会自动推导（默认 github.io 域名）或用 `&repo=你的owner/你的repo` 显式指定。每个 fork 用各自的 Token，不要共用。
