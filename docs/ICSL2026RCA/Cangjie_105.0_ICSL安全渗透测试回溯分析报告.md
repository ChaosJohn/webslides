...
> 版本：Cangjie 105.0　|　测试方：ICSL 华为内网安全实验室　|　报告日期：2026年9月

---

## 一、背景与概述

ICSL（华为内网安全实验室）对 Cangjie 105.0 版本开展白盒安全渗透测试，共发现 10 个安全问题，覆盖编译器宏系统、HLE 工具链、调试器、标准运行时库（std）及扩展库（stdx）等模块。所有问题均已由仓颉团队完成修复并提交对应 Pull Request。

本报告聚焦三个方面：内部检测能力差距分析、测试看护缺口识别、系统性改进措施（5W2H）。
...
下图以攻击路径可达性为横轴、潜在影响程度为纵轴，呈现 10 个问题的分布矩阵，不同颜色代表所属模块。

![攻击路径与影响矩阵](attack_path_matrix.png)

**图1　Cangjie 105.0 ICSL 渗透测试问题 — 攻击路径与影响矩阵**
...
| 问题 | 对应模式 | 模块审计状态 |
|------|---------|------------|
| #9 HTTP chunked 绕过大小限制 | HTTP-002 Chunked Body No Size Limit | stdx_net_http 已完成审计（35 findings） |
