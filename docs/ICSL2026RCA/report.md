...
| 编号 | 模块 | 问题简述 | 攻击路径分类 | 控制点 | 改进方 |
|------|------|----------|-------------|--------|--------|
| 1 | Macro | FlatBuffers Verifier 缺失，LSP 宏服务器 IPC 消息未校验 | 有路径·需本地权限 | 代码检视 | 开发 |
| 2 | HLE | 注释闭合突破注入，恶意 .d.ts 可注入可执行代码 | 有路径·需用户交互 | 代码检视 | 开发 |
| 3 | cjdb | int8_t 循环变量溢出导致无限循环和越界读取 | 无攻击路径 | 代码检视/静态检查 | 开发 |
| 4 | std | IPv6 字面量解析器未兼容 RFC 4291 允许的格式 | 无攻击路径 | DT测试、协议测试 | 开发测试 |
| 5 | std | CJ_FS_OpenFile 符号链接跟随（TOCTOU 竞态） | 有路径·需本地权限 | 代码检视 | 开发 |
| 6 | stdx | crypto/keys 加密路径密码未清零，堆内存残留 | 有路径·需本地权限 | 代码检视 | 开发 |
| 7 | stdx | crypto/keys PKCS8 加密路径 algorithm 对象泄漏 | 无攻击路径 | 代码检视/静态检查 | 开发 |
| 8 | stdx | crypto/digest SM3 失败路径悬空指针导致二次释放 | 无攻击路径 | 代码检视/静态检查 | 开发 |
| 9 | stdx | HTTP/1.1 chunked 请求体绕过配置的请求大小限制（1:1攻击） | 有路径·远程可达 | 测试/代码检视 | 开发、测试 |
| 10 | stdx | Tar 解压经父级符号链接删除目标目录之外的文件 | 有路径·需本地权限 | 代码检视 | 开发 |
...
ICSL 完全复用我们的三个挖洞 Skills（cangjie-vuln-hunter、audit-lite、Cangjie-compiler-vuln-hunter），模式库、扫描流程、误报排除规则均相同。我们未发现而 ICSL 发现的差异来自五个方面：模型组合与围栏应对、模式库执行效果、问题定级标准、问题探索深度、投入资源与考核动机。
...
本次 ICSL 对 Cangjie 105.0 的渗透测试属于保障项目，其投入资源和考核动机与日常安全扫描存在差异。
...
1. **结项汇报压力**：ICSL 后续需做结项汇报，存在一定考核压力。若发现问题数量过少，可能被认为渗透测试工作不充分。
2. **项目经理因素**：本次 ICSL 参与仓颉保障项目的项目经理为新人，存在横向考核指标（与其他项目经理对比），有动力产出更多可汇报的发现。

**对回溯分析的影响：**

投入资源与考核动机是上述各维度差异的背景因素。保障项目的集中式投入（6 人 × 20 天）与日常分散式扫描在投入强度上存在量级差异；结项汇报和横向考核的压力，使得 ICSL 在问题挖掘和上报上更具主动性。
...
| 维度 | 内容 |
|------|------|
| **What** | 送检前使用最新的 SOTA 模型作为白盒扫描主力模型；关注最新的越狱机制，建立安全围栏应对策略库 |
| **Why** | 我们使用 GPT 5.5，ICSL 使用 GPT 5.6，模型版本差异客观存在；Claude 因安全围栏在我们的扫描中基本不可用，ICSL 通过越狱突破了 GPT 5.6 的围栏。本次 10 个问题中 5 个已有模式但未检出，说明模式库本身覆盖到位，差异在模型执行效果。送检前使用最新 SOTA 模型并建立围栏应对策略是提升扫描检出能力的必要措施 |
| **Who** | 虞嘉豪 |
| **When** | 每次使用国外SOTA模型进行安全扫描时 |
| **Where** | 送检前模型版本评估；安全围栏应对策略 |
| **How** | ① 建立送检前模型版本评估机制，确保使用当时最新的 SOTA 模型；② 持续关注并跟踪最新的越狱机制，更新围栏应对策略库；③ 对围栏中断的扫描任务自动降级到备用模型或调整提示词重试 |
...
| **Why** | 本次 10 个问题中 3 个模式覆盖不足、2 个真正盲区。主要差距在于模式覆盖边界过窄和部分组件缺少特有模式，而非模式库整体缺失 |
| **Who** | 虞嘉豪 |
| **When** | 9.30 |
| **Where** | 优化后的挖洞Skills |
...
| 维度 | 内容 |
|------|------|
| **What** | 安全问题定级严格按照 CVE 的 CVSS 打分标准执行 |
| **Why** | 本次发现定级标准差异导致部分问题被判定为非问题：#9 HTTP chunked 无大小限制因 1:1 攻击判定为非问题，ICSL 判定为低危；#2 HLE 注释闭合注入因"开发者应对工具生成代码负责"判定为非问题，ICSL 判定为严重。 |
| **Who** | 虞嘉豪 |
| **When** | 10.30 |
| **Where** | 安全问题定级标准参考文档 |
| **How** | 向 ICSL 交流学习 CVSS 打分细则，建立内部打分规范，输出文档 |

### 措施4

| 维度 | 内容 |
|------|------|
| **What** | 问题所属的责任组单独做回溯分析，整理典型案例 |
| **Why** | 从开发源头预防同类问题，提升开发团队的安全编码意识。10 个问题覆盖反序列化校验、敏感数据清零、符号链接处理、资源释放、类型匹配、协议兼容、请求大小限制、归档路径安全等多个开发场景，具有典型参考价值 |
| **Who** | 问题所属责任组 |
| **When** |  |
| **Where** | 各责任组回溯分析报告；安全编码典型案例库 |
| **How** | ① 排查还有没有类似的其他问题；② 做泛化分析，评估同类问题在其他模块/场景的潜在影响；③ 整理典型案例，说明问题的成因、危害和避免方法；④ 明确每个问题应该在什么阶段防住（DT、代码检视、测试等） |

> **HLE 问题发散示例**：#2 HLE 注释闭合突破注入的根因是代码生成过程中对不可信输入未做转义。责任组回溯分析时应发散排查 HLE 工具中其他类似的代码生成/字符串拼接逻辑，例如操作符处理（`+`/`-`）等环节是否也存在类似的注入风险，避免仅修复单点而遗漏同类问题。

### 措施5

| 维度 | 内容 |
|------|------|
| **What** | 从安全测试角度做改进，基于当前测试人力选择重点落脚点 |
| **Why** | 现有测试体系存在看护缺口：错误路径/失败路径测试不足（#7、#8）、边界条件测试不足（#3）属于安全测试缺口；协议兼容性测试不足（#4）、操作顺序测试不足（#10）本质上是功能测试不足。但测试人力有限，无法全面铺开，需要选择重点方向投入 |
| **Who** | 测试团队 |
| **When** | 10.30 |
| **Where** | 安全测试改进 |
| **How** | 基于当前测试人力评估，选择1~2个缺口作为 2026 年测试改进落脚点（选择逻辑：可行性+性价比） |
